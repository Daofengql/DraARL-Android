...
| JVM 单元测试 Kotlin | 75 | 6,744 | 300 个测试用例 |
...
- `RadioConnectionService` 的待绑定消息队列和监听器槽位已提取为 `RadioMessageBuffer` / `RadioMessageDispatcher`；FIFO、满载淘汰、清空、非法容量、绑定排空和解绑后重新缓存由 6 个 JVM 用例保护，Service 仍在原子边界外回调监听器。
