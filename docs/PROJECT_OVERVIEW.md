...
| 范围 | 文件数 | 代码行数 | 说明 |
| --- | ---: | ---: | --- |
| 生产 Kotlin | 197 | 28,850 | 不含空行、生成目录和第三方源码 |
...
| Compose 截图测试 Kotlin | 7 | 1,417 | 48 张壳层、页面、状态和组件参考图 |
...
- 应用启动与壳层、五个一级页面、页面顶部栏、趋势空态、更新反馈、认证与账号安全反馈、设置行、APRS、BLE 配网、存储页、工具子页、弹窗、Bottom Sheet 和首批页面反馈已有 48 张可重复生成的浅色/深色参考图，覆盖窄屏、常规手机、横屏、长中文、强制更新下载进度、设备与群组筛选空态、设备绑定成功凭据、BLE 设备选择空态、通联日志筛选空态、密码重置成功、邮箱修改失败、中继查询无结果与结果列表、存储清理禁用态以及 1.3/1.5/2.0 倍字体。
