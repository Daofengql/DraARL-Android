...
| 范围 | 文件数 | 代码行数 | 说明 |
| --- | ---: | ---: | --- |
| 生产 Kotlin | 185 | 27,973 | 不含空行、生成目录和第三方源码 |
| JVM 单元测试 Kotlin | 64 | 5,226 | 263 个测试用例 |
...
1. `DevicesScreen` 仍超过 1,000 行；`AppController` 为 988 行，`UdpRadioClient` 为 951 行。UDP 状态、认证、Socket 传输、心跳监测、PTT 编排、接收语音组装、任务调度和音频设备边界已经独立，后续重点转向普通异步任务所有权和大型页面。
