...
| 范围 | 文件数 | 代码行数 | 说明 |
| --- | ---: | ---: | --- |
| 生产 Kotlin | 189 | 28,330 | 不含空行、生成目录和第三方源码 |
| JVM 单元测试 Kotlin | 71 | 6,331 | 285 个测试用例 |
| Android 仪器测试 Kotlin | 3 | 94 | 主要覆盖底部导航和 SQLite |
| Compose 截图测试 Kotlin | 6 | 1,005 | 32 张壳层、页面、状态和组件参考图 |
...
- 应用壳层、五个一级页面、认证反馈、设置行、APRS、BLE 配网、存储页、工具子页、弹窗、Bottom Sheet 和首批页面反馈已有 32 张可重复生成的浅色/深色参考图，覆盖窄屏、常规手机、横屏、长中文以及 1.3/1.5/2.0 倍字体。
