...
| 范围 | 文件数 | 代码行数 | 说明 |
| --- | ---: | ---: | --- |
| 生产 Kotlin | 186 | 27,978 | 不含空行、生成目录和第三方源码 |
| JVM 单元测试 Kotlin | 65 | 5,351 | 266 个测试用例 |
...
- 设备、群组和资料 Controller 的普通阻塞任务由共享 `ControllerTaskRunner` 承载；每个 Controller 持有挂接到 `viewModelScope` 的子 `SupervisorJob`，阻塞调用显式切到 IO dispatcher，重置或关闭后取消任务并丢弃迟到结果。
...
1. `DevicesScreen` 仍超过 1,000 行；`AppController` 为 988 行，`UdpRadioClient` 为 951 行。UDP 状态、认证、Socket 传输、心跳监测、PTT 编排、接收语音组装、任务调度和音频设备边界已经独立，设备、群组和资料 Controller 已建立结构化任务所有权，后续重点转向其余普通异步任务和大型页面。
...
- 设备、群组和资料 Controller 已移除各自的 `Executor`、`Handler`、原子关闭标记和代次模板，统一通过 `ControllerTaskRunner` 管理 loading、IO 调度、取消和主 scope 回投；3 个 JVM 用例覆盖 dispatcher、迟到结果和关闭边界。
