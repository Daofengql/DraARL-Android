...
这是一个中等规模、功能面较宽的单模块 Android 客户端。自研生产 Kotlin 约 2.79 万有效代码行，已经覆盖账号、设备、群组、实时通信、地图、APRS 和多种业余无线电工具；复杂度主要来自通信状态、音频生命周期、后台服务和硬件/系统权限，而不是 Gradle 模块数量。

| 范围 | 文件数 | 代码行数 | 说明 |
| --- | ---: | ---: | --- |
| 生产 Kotlin | 184 | 27,865 | 不含空行、生成目录和第三方源码 |
| JVM 单元测试 Kotlin | 63 | 5,113 | 259 个测试用例 |
...
| `radio` | 41 | 消息状态与同步、会话、UDP、音频、重连、缓存和前台通信服务 |
...
1. `DevicesScreen` 仍超过 1,000 行；`AppController` 为 988 行，`UdpRadioClient` 已降至 950 行。UDP 状态、认证、Socket 传输、心跳监测、PTT 编排、接收语音组装、任务调度和音频设备边界已经独立，后续重点是收敛会话状态的串行修改上下文、普通异步任务所有权和大型页面。
...
- UDP 连接、认证、在线、重连、错误、主动断开与关闭已建模为显式状态和事件，单一不可变快照取代分散的目标配置、手动断开、代次和重连原子量；44 个新增 JVM 用例覆盖状态顺序、陈旧事件、重复重连、认证响应、总超时、Transport 收发/关闭、心跳与静默边界、调度任务所有权、PTT 音频边界及接收流乱序/淘汰/结算，资源所有权与关闭顺序记录在 `docs/UDP_CONNECTION_LIFECYCLE.md`。
...
- 应用壳层、五个一级页面、设置行、APRS、BLE 配网、存储页、弹窗、Bottom Sheet 和首批页面反馈已有 24 张可重复生成的浅色/深色参考图，覆盖窄屏、常规手机、横屏、长中文以及 1.3/1.5/2.0 倍字体。
