...
- 每次新连接、重连调度、主动断开或关闭都会推进代次。Socket、调度任务和音频回调只有在代次仍匹配时才能更新连接状态。
...
| Socket 创建与认证握手 | `draarl-udp-connect` 单线程执行器 | 仅当前代次可以安装和激活 Socket |
| UDP 阻塞接收 | 每次在线会话的 `draarl-udp-receiver` | 关闭对应 Socket 或代次失效后退出 |
