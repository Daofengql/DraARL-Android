...
后续将邮箱修改失败反馈提取为可直接截图的生产组件后，再次强制全量编译的当前报告为 908 个 Composable、900 个可重启、
618 个可跳过，已知稳定参数为 13,344 个。新增数量来自生产反馈组件及其截图入口，不改变上述状态读取边界，也不代表运行时
性能边界退化；真机重组次数和帧时间仍需按下节验证。
