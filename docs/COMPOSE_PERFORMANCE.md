...
后续完成反馈组件提取、选择弹窗状态投影和动态列表 key 修正后，再次强制全量编译的当前报告为 908 个 Composable、900 个
可重启、617 个可跳过，已知稳定参数为 13,346 个。`AccessPointDialogState`、`RoutingDialogState`、`AmapPlace` 和
`RegionOption` 均被识别为稳定类型，两个弹窗及地区选择组件均保持可跳过。汇总数量会随生产组件签名和编译器生成作用域变化，
不直接代表运行时性能边界；真机重组次数和帧时间仍需按下节验证。

## 选择弹窗状态边界

节点选择和收听频道弹窗原先直接接收完整 `RadioSessionUiState`。语音收发期间，`speaker`、`transmitting`、端点和瞬态错误
会替换完整状态，即使节点弹窗只显示节点、探测和当前选择，频道弹窗只显示主频道、收听集合、连接和更新状态。

现在两个 Controller 包装层分别通过带 `structuralEqualityPolicy()` 的 `derivedStateOf` 发布不可变
`AccessPointDialogState` 和 `RoutingDialogState`。无关电台状态更新不再发布新投影；4 个 JVM 用例使用 Snapshot 观察者验证
无关更新失效次数为 0，并逐项覆盖所有会改变弹窗显示的字段。

## 动态列表身份

全量审计覆盖 27 个 Lazy 容器，其中 18 个承载动态业务数据。地区选择是唯一未提供 key 的业务列表，现使用资源内唯一的地区
代码；地点搜索原先使用经纬度，同坐标多个 POI 会产生重复 key，现优先使用高德 POI ID，并以带命名空间、长度与坐标 bit 值
的复合 key 兜底，服务层同时去除完全重复的结果。其余动态列表已经使用实体 ID、消息 ID、设备地址或枚举目的地作为稳定 key；
动态行模型也均被 compiler report 识别为稳定类型。

## 验证边界

本批已通过 Debug Kotlin 编译和 Compose compiler 报告对比。本地 Android 17（API 37.1）模拟器已执行消息缓存和
大字体弹窗仪器测试；代码审计未发现其他会被持续高频无关状态驱动的页面订阅，动态业务列表也已全部具备稳定且无冲突的 key。
尚未使用真机 Layout Inspector 验证 PTT 收发期间的实际重组次数，也未记录帧时间，该运行时证据继续由 `TODO.md` 跟踪。
