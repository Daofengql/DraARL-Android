...
当前 48 张参考图在 Windows 本地通过 JVM/Layoutlib 渲染，不依赖模拟器、真机或远程服务。
...
| 工具 | 800 x 360 / 360 x 640 / 360 x 800 dp | 360 x 700 / 411 x 891 / 360 x 360 dp | 横屏、账号审核、工具子页加载态、1.3 倍字体中继结果列表，以及 1.5 倍字体错误、空态、无结果和通联日志筛选空态 |
...
- 通联日志、电台预设和中继查询使用同一工具标题、命令按钮及列表反馈层级；中继可用状态复用 `StatusIndicator`，不再用页面自定义颜色文字表达，长名称、频率、亚音、维护信息和停用状态已有结果列表基线，通联日志按呼号筛选无结果已有独立基线。
...
- 本文证明一级页面及首批认证/账号安全反馈、设置行、APRS、BLE 配网、存储页、工具子页、错误提示、弹窗和 Bottom Sheet 静态基线已建立；设备与群组空/加载态、设备筛选空态、设备绑定成功凭据、BLE 设备选择空态、通联日志筛选空态、密码重置成功、PTT 同步错误、应用通知、工具加载与工具错误/空态已有首批页面状态基线，剩余状态矩阵仍由
