...
当前 50 张参考图在 Windows 本地通过 JVM/Layoutlib 渲染，不依赖模拟器、真机或远程服务。

| 场景 | 浅色 | 深色 | 额外覆盖 |
| --- | --- | --- | --- |
| 应用壳层 | 360 x 800 dp | 411 x 891 dp | 五入口底栏、突出 PTT 入口，以及 1.3 倍字体长通知 |
| 启动恢复 | 360 x 640 dp | - | 静态品牌、通信会话恢复状态和 1.3 倍字体 |
| 设备 | 360 x 800 dp | 411 x 891 / 360 x 800 / 360 x 640 dp | 长设备名、在线/离线、受限状态、1.3 倍字体加载态，以及 1.5 倍字体页面空态、筛选空态和绑定成功凭据 |
| 群组 | 360 x 800 dp | 411 x 891 / 360 x 800 dp | 公开/私有分区、长中文群组名、空态、筛选空态和 1.3/1.5 倍字体 |
| PTT | 411 x 891 / 360 x 800 dp | 360 x 800 / 360 x 180 dp | 状态条、频道、空日志、1.5 倍字体的初始同步失败、历史分页错误和发送控制 |
| 工具 | 800 x 360 / 360 x 640 / 360 x 800 / 360 x 360 dp | 360 x 700 / 411 x 891 / 360 x 360 dp | 横屏、账号审核、工具子页加载态、1.3 倍字体中继结果列表，以及 1.5 倍字体错误、空态、无结果、通联日志筛选空态和电台预设空态 |
...
| 认证与账号安全 | 360 x 560 / 320 x 360 dp | 360 x 300 / 360 x 520 / 320 x 420 dp | 1.3 倍字体注册成功/一次性密码、1.5 倍字体错误/忙碌态，以及 2.0 倍字体密码重置成功和邮箱修改失败反馈 |
...
- 设备绑定成功结果将 SSID、UDP 账号、设备密码和 DMR ID 收敛为独立生产组件；深色 360 x 640 dp、1.5 倍字体基线覆盖长凭据和两个复制命令的稳定布局。
...
- 通联日志、电台预设和中继查询使用同一工具标题、命令按钮及列表反馈层级；中继可用状态复用 `StatusIndicator`，不再用页面自定义颜色文字表达，长名称、频率、亚音、维护信息和停用状态已有结果列表基线，通联日志按呼号筛选无结果与电台预设首次空态均有独立基线。
- 地点选择和梅登海德网格页的可恢复错误复用 `ToolError` / `InlineNotice`；嵌入地图面板时沿用面板留白，不额外嵌套默认 Material 错误容器。
- PTT 空日志和同步失败使用 `PageFeedback`，列表顶部的历史加载与同步失败使用 `InlineNotice`；截图直接覆盖生产反馈组件，并包含有历史记录时的分页同步失败，不再手工拼装近似空态。
...
- 认证错误、注册成功和密码重置成功使用统一反馈层级；登录、注册和重置的主要动作使用支持忙碌态的命令按钮，一次性准入密码使用警告容器与等宽数据样式；密码重置后的成功提示和返回登录动作已有 2.0 倍字体深色基线。
- 账号安全页一次只展开密码或邮箱表单；发送验证码、重新获取和确认修改使用同一命令层级，邮箱错误继续复用认证错误提示。邮箱提交反馈直接组合生产 `EmailChangeSubmission`，浅色 320 x 360 dp、2.0 倍字体基线覆盖长失败信息的换行和操作可见性。
...
- 本文证明一级页面及首批认证/账号安全反馈、设置行、APRS、BLE 配网、存储页、工具子页、错误提示、弹窗和 Bottom Sheet 静态基线已建立；设备与群组空/加载态、设备筛选空态、设备绑定成功凭据、BLE 设备选择空态、通联日志筛选空态、电台预设空态、密码重置成功、PTT 初始与分页同步错误、应用通知、工具加载与工具错误/空态已有首批页面状态基线，真机状态矩阵仍由
