...
class ToolApiJsonTest {
    @Test
    fun `relay parser follows server legacy field names`() {
        val relay = ToolApiJson.relay(
            JSONObject(
                """{"id":7,"name":"广州中继","up_freq":"439.500","down_freq":"438.500","send_ctss":"88.5","recive_ctss":"88.5","ower_callsign":"BG7TEST","location":"广东省 广州市","status":1,"note":"测试"}""",
            ),
        )
    ...
    fun `logbook parser preserves nullable powers and station details`() {
        val logbook = ToolApiJson.logbook(
            JSONObject(
    ...
    fun `preset parser maps server ordering`() {
        val preset = ToolApiJson.preset(
            JSONObject("""{"id":3,"name":"车台","radio":"FTM-500","antenna":"SG7900","power":50,"qth":"深圳","sort_order":2}"""),
        )
