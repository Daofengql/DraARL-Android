...
- 生产 Kotlin 194 个文件，约 2.86 万有效代码行；JVM 测试 81 个文件、314 个用例。
- Compose 截图测试 6 个文件、42 张参考图，覆盖启动与应用壳层、五个一级页面、页面顶部栏、趋势空态、更新反馈、认证与账号安全反馈、设置行、APRS、BLE 配网、存储页及清理反馈、工具子页、中继查询无结果与结果列表、弹窗、Bottom Sheet 以及设备和群组筛选空态。
...
- `ProfileController.reset()` 的迟到结果，以及 `PublicAuthController`、`DashboardCacheController`、`GroupManagementController`、`SessionController`、`AppUpdateController`、`ToolsController` 和 `RadioSessionController` 关闭后的迟到回调。
