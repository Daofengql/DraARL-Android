...
- 生产 Kotlin 189 个文件，约 2.83 万有效代码行；JVM 测试 71 个文件、285 个用例。
- Compose 截图测试 5 个文件、27 张参考图，覆盖应用壳层、五个一级页面、设置行、APRS、BLE 配网、存储页、工具子页、弹窗、Bottom Sheet 和首批页面反馈状态。
...
- 继续补齐设备空/错、群组加载/错以及其他一级页面仍可到达的状态组合；PTT 空态和同步错误态已覆盖。
- 扩大非默认字体缩放对五个一级页面的覆盖。
