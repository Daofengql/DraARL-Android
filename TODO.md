...
- 生产 Kotlin 188 个文件，约 2.83 万有效代码行；JVM 测试 71 个文件、285 个用例。
- Compose 截图测试 5 个文件、26 张参考图，覆盖应用壳层、五个一级页面、设置行、APRS、BLE 配网、存储页、工具子页、弹窗、Bottom Sheet 和首批页面反馈状态。
...
2. 真机验证异步生命周期，继续分离 `UdpRadioClient` 的传输与音频职责并补确定性测试。
