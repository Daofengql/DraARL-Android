...
1. `AprsController`：配置、手动发送和后台服务协调。
2. `RadioMessageController`：缓存、分页、同步、对账和公开资料预加载。
3. `RadioSessionController`：节点发现、连接、路由和 Service Binder。
4. `SessionController`：登录态恢复、刷新和退出清理。
...
位置：`ui/theme/Theme.kt` 和 `ui/components/`。

- 保留现有浅色/深色方案，进一步区分中性背景、通信状态、告警、数据和交互强调色。
- 降低默认 Material 大圆角和胶囊形态的出现频率；普通容器优先使用 0-6 dp 圆角。
- 技术数据、呼号、频率、延迟和计时使用稳定的等宽或表格数字样式。
- 统一页面间距、分隔线、图标尺寸、触控高度和紧凑/常规信息密度。
- 系统动态取色保持关闭或明确设为可选，避免品牌色随壁纸漂移。

验收：页面不直接依赖默认 Material shape/颜色表达业务状态，浅色和深色均满足对比度要求。

- [ ] **优化应用壳层和一级页面**

- 保留五入口与现有导航行为，弱化默认 NavigationBar 的悬浮胶囊和大面积单色填充感。
- PTT 入口继续突出，但使用设备控制语义，而不是普通品牌主按钮样式。
- 顶栏、底栏、内容区和系统 Insets 形成稳定层级，横竖屏不跳动。
- 设备、群组和工具页优先使用紧凑列表、分区标题、细分隔线和明确的主次动作。
- 个人页使用无框身份区和设置列表，避免把整段页面包进多个等权 Card。

验收：信息密度更适合反复操作，一级页面看起来属于同一产品，同时仍符合 Android 导航习惯。
