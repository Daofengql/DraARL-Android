...
- 生产 Kotlin 170 个文件，约 2.67 万有效代码行；JVM 测试 55 个文件、215 个用例。
- Compose 截图测试 3 个文件、17 张参考图，覆盖应用壳层、五个一级页面、设置行、存储页和弹窗。
...
- Bottom Sheet、空状态、加载、错误和权限拒绝统一标题、反馈层级与操作位置。
...
- 为 Bottom Sheet 建立组件截图基线。
- 为五个一级页面覆盖空、错、加载和典型数据状态。
- 补充 1.3 倍字体基线，并扩大 1.5/2.0 倍字体对一级页面和 Bottom Sheet 的覆盖。
...
2. 显式化 `UdpRadioClient` 状态机，统一普通异步任务并补确定性测试。
